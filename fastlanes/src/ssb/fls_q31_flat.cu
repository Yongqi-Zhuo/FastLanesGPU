...
inline auto query_mtd = ssb::ssb_q31_100;
...
__global__ void probe_v4(int* lo_orderdate,
	...
	BlockPredAndEQ<int, BLOCK_THREADS, ITEMS_PER_THREAD>(items, 2, selection_flags, num_tile_items);
	...
	constexpr int year_min = 1992;
	constexpr int year_max = 1997;
	#pragma unroll
	for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
		int logical_idx = threadIdx.x + (BLOCK_THREADS * ITEM);
		if (logical_idx < num_tile_items) {
			int actual_year = (items[ITEM] + query_mtd.ssb.lo_orderdate_min) / 10000;
			years[ITEM]     = actual_year;
			if (actual_year < year_min || actual_year > year_max) { selection_flags[ITEM] = 0; }
		}
	}
	...
	BlockPredLoad<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
	    lo_s_nation + tile_offset, s_nation, num_tile_items, selection_flags);
	BlockPredLoad<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
	...
	for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
		...
		if (logical_idx < num_tile_items && selection_flags[ITEM]) {
			int hash = (s_nation[ITEM] * 25 * 7 + c_nation[ITEM] * 7 + (years[ITEM] - 1992)) %
			           ((1998 - 1992 + 1) * 25 * 25);
			res[hash * 6]     = years[ITEM];
...
void runQuery(int*                         lo_orderdate,
	...
	if (version == 4) {
		probe_v4<BLOCK_THREADS, ITEMS_PER_THREAD><<<(lo_len + tile_items - 1) / tile_items, BLOCK_THREADS, 0, stream>>>(
		    lo_orderdate, lo_c_region, lo_s_region, lo_c_nation, lo_s_nation, lo_revenue, lo_len, res);
	} else {
...
int main(int argc, char* argv[]) {
	...
	int* h_lo_s_nation   = loadColumn<int>("lo_s_nation", LO_LEN);
	...
	int* d_lo_c_region  = loadToGPU<int32_t>(h_enc_lo_c_region, hard_coded.n_tup_line_order, g_allocator);
	...
	int* d_lo_s_nation  = loadToGPU<int32_t>(h_lo_s_nation, hard_coded.n_tup_line_order, g_allocator);
