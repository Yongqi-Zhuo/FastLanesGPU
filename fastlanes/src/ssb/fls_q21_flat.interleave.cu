...
__global__ void probe_v3(int* lo_orderdate,
	...
	LoPair brand_revenue[ITEMS_PER_THREAD];
    ...
    BlockPredLoad<LoPair, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
        lo_pair + tile_offset, brand_revenue, num_tile_items, selection_flags);
	...
	for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
		if ((threadIdx.x + (BLOCK_THREADS * ITEM)) < num_tile_items) {
			if (selection_flags[ITEM]) {
				...
				int brand = brand_revenue[ITEM].lo_p_brand1;
				int revenue = brand_revenue[ITEM].lo_revenue;
				int hash          = (brand * 7 + (year - 1992)) % ((1998 - 1992 + 1) * (5 * 5 * 40));
				res[hash * 4]     = year;
				res[hash * 4 + 1] = brand;
				atomicAdd(reinterpret_cast<unsigned long long*>(&res[hash * 4 + 2]), (long long)(revenue));
			}
		}
	}
}
...
void runQuery(int*                         lo_orderdate,
	...
	if (version == 3) {
		probe_v3<BLOCK_THREADS, ITEMS_PER_THREAD><<<(lo_len + tile_items - 1) / tile_items, BLOCK_THREADS, 0, stream>>>(
		    lo_orderdate, lo_p_category, lo_s_region, lo_pair, lo_len, res);
	} else {
...
int main(int argc, char* argv[]) {
	...
	auto *h_lo_pair = new LoPair[LO_LEN];
	for (size_t i {0}; i < LO_LEN; ++i) {
		h_lo_pair[i].lo_p_brand1 = h_lo_p_brand1[i];
		h_lo_pair[i].lo_revenue = h_lo_revenue[i];
	}
	...
	LoPair* d_lo_pair = loadToGPU<LoPair>(h_lo_pair, hard_coded.n_tup_line_order, g_allocator);
