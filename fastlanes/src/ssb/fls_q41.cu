...
void runQuery(int*                         lo_orderdate,
	...
	{
		auto path = std::filesystem::path(DATA_DIR "benchmark/fastlanes_gpu/q41.txt");
...
int main(int argc, char* argv[]) {
	...
	cudaMallocHost(&h_enc_lo_partkey, n_vec * 1024 * sizeof(int));
	const int* h_enc_lo_supplycost;
	cudaMallocHost(&h_enc_lo_supplycost, n_vec * 1024 * sizeof(int));
	...
	int* d_lo_supplycost = loadToGPU<int32_t>(h_enc_lo_supplycost, supplycost_out - h_enc_lo_supplycost, g_allocator);
	...
	auto benchH2D = casdec::benchmark::benchmarkKernel(
		[&] {
			...
			loadToGPUBuffer(h_enc_lo_partkey, partkey_out - h_enc_lo_partkey, d_lo_partkey, stream);
			loadToGPUBuffer(h_enc_lo_supplycost, supplycost_out - h_enc_lo_supplycost, d_lo_supplycost, stream);
			loadToGPUBuffer(h_d_datekey, D_LEN, d_d_datekey, stream);
			loadToGPUBuffer(h_d_year, D_LEN, d_d_year, stream);
			loadToGPUBuffer(h_p_partkey, P_LEN, d_p_partkey, stream);
			loadToGPUBuffer(h_p_mfgr, P_LEN, d_p_mfgr, stream);
	...
	auto bandwidthH2D = (orderdate_out - h_enc_lo_orderdate +
						...
						supplycost_out - h_enc_lo_supplycost +
						2 * D_LEN +
						2 * P_LEN +
						2 * S_LEN +
						3 * C_LEN
